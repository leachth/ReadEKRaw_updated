# Create data matrix for plotting and analysis. 
  # data              		must be a data object that comes out of ReadEKRaw(fname) & ReadEKRaw_ConvertPower(data). 
                      		#If matrix is to be constrained above the lake bottom then the data object must 
                      		# have also been processed by the FindBottom2(data). 
                      		# Currently handles data objects with 710 kHz frequency 
  # nf                		nearfield distance, offset below surface in meters  
  # use.bottom.line   		Use the lake bottom to bottom depth of the matrix; default is FALSE
  # bottom.line.offset      depth offset from bottom in meters, default is 0 (can also be a negative number)
  # threshold         		data with backscatter (dB) stronger than this value are turned to NA's 
  							# Use this to remove possible scatter from fish
