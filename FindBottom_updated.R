...
# discrimination level - any ping with a dB greater than this value is likely the bottom; Echoview defualt is -50 dB
# create empty vector to store bottom depth for each ping
# http://support.echoview.com/WebHelp/Echoview.htm

# FindBottom
# Input: A ReadEKRaw_ConvertPower data object (i.e., you must run ReadEKRaw and then ConvertPower first). A list of three objects called data,
# (1) File header, (2) pings, (3) GPS data. These three objects are themselves lists. 

# Output: adds a vector named bot.depth to each object in data$pings. Also generates a new element to data
# called bottom.line  that is a vector of the bottom depths at all indexed value in 1:length(data$pings),
# this vector is created for ease when plotting or other analyses.
...
# must load zoo package first
library("zoo")

FindBottom2 = function(data, nf = 1, d = -30, smooth = TRUE){
